...
**Freigabe-Workflow:** `pipeline test` → `pipeline engineering` → `pipeline fit`
→ physische Qualifikation → `pipeline release`. Rechnerisches PASS ohne
physische Nachweise führt nur zu `PROTOTYPE_ONLY`.
